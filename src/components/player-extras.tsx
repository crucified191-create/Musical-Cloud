...
export function PlayerExtras() {
  ...
  return <>
    {isInstalled("queue-view") ? <button type="button" onClick={() => setQueueOpen(true)} className="fixed bottom-24 right-5 z-30 rounded-full border border-neutral-700 bg-neutral-950 px-4 py-2 text-sm hover:border-[var(--accent)]">Queue</button> : null}
    {isInstalled("fullscreen-player") ? <button type="button" onClick={() => setFullscreen(true)} className="fixed bottom-24 right-24 z-30 rounded-full border border-neutral-700 bg-neutral-950 px-4 py-2 text-sm hover:border-[var(--accent)]">Fullscreen</button> : null}
